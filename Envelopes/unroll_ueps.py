...
#
#   @ Get the envelope for WW UE/PS Uncertainty
#
...
remapList = get_remap_config("cfg/Spin2p_min_0j_10x5_log_remapped_v31.cfg")    #### change the cfg file in


# STEP 2 ) GET THE UNROLLED DISTRIBUTIONS
py6_unroll = get_unrolled("../MakeSelections/histfiles/py6_hists/final_pythia6_0jet.root","bdt2_spin0p_spin2pmin_0jet","py6")
#amc_unroll = get_unrolled("../MakeSelections/histfiles/amc_hists/final_herwig_0jet.root","bdt2_spin0p_spin2pmin_0jet","herwig")  #### carefull about jets and bdt
her_unroll = get_unrolled("../MakeSelections/histfiles/her_hists/final_herwig_0jet.root","bdt2_spin0p_spin2pmin_0jet","herwig")
#
# this code already works by getting the ratio of amc/her, just change the input files to get her/py6
#
...
for ibin in range(1, amc_remap.GetNbinsX()+1):
    ...
    py6_remap_content.append(py6_remap.GetBinContent(ibin))
...
ratio = her_remap.Clone()
ratio.Sumw2()
ratio.Divide(py6_remap)

canvas_remap = rt.TCanvas("remap","remap",800,600)
canvas_remap.cd()
rt.gStyle.SetOptStat(0)
ratio.SetTitle("")
ratio.GetYaxis().SetTitle("Herwig/Pythia")
ratio.GetXaxis().SetTitle("remapped BDT")
ratio.GetYaxis().SetRangeUser(0.75, 1.15)
ratio.SetLineColor(rt.kBlack)
ratio.SetLineWidth(3)
ratio.Draw()
canvas_remap.SaveAs("outPlots/UEPS/test.pdf")
...
if do_full_log:
    fullLog=open('fullLogs/UEPS/ueps_spin2pkg05kq1_hpt300_0jet.dat','w')  #### change the name of the full log file
    fullLog.write('# ratio_content , ratio_error , her_remap_content , pythia_remap_content\n')
    for i in range(len(content1)):
        fullLog.write('%0.10f\t%0.10f\t%f\t%f\n' % (math.fabs(1.-content1[i]), error1[i], her_remap_content[i], py6_remap_content[i]))
...
for i in range(len(content1)):
    ...
    print math.fabs(1.-content1[i]) ,her_remap_content[i]
# --- write output file
if write_file:
    outfile = open('outFiles/UEPS/ueps_spin2pkg05kq1_HPt300_0jet_v31.dat','w')  #### change the name of the file
    outfile.write('# ueps spin2pmin 1jet\n')
...
if do_plot:
    ...
    leg.SetFillColor(0)
    leg.SetNColumns(1)
    leg.AddEntry(h_up, "WW Shape Up", "lep")
    leg.AddEntry(h_nom, "Nominal", "lep")
    leg.AddEntry(h_dw, "WW Shape Down", "lep")
    ...
    canvas.SaveAs("outPlots/UEPS/ueps_Spin2pkg05kq1_HPt300_0j.pdf")  #### change the name of the plot
